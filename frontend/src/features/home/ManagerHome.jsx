...
export default function ManagerHome({ selectedEntity = "all", onNavigate }) {
  ...
  // Papan antrean mahal — pemilihannya SATU fungsi bersama dengan Control Tower
  // (`selectWaitingBoards`): saat data tak terbaca papan tetap digambar supaya
  ...
  return (
    <div className="grid gap-3" data-testid="manager-home">
          ...
          {loading && !data ? (
            ...
            <>
              <div className="grid grid-cols-2 gap-2 md:grid-cols-5"
                ...
                <Kpi label="Menunggu tanda tangan"
        ...
        <section className="section-card">
          ...
          <div className="section-body">
            {loading && !data ? (
              ...
              /* B5 — cek ini WAJIB di atas cek panjang daftar: saat pemuatan ulang
                 gagal, `data` masih memegang isi lama sehingga daftar TIDAK kosong dan
                 layar dulu menyajikan antrean BASI dengan percaya diri. Gagal memuat
                 juga ≠ meja bersih. */
              <p className="py-8 text-center text-[11.5px] text-[#C0392B]"
                data-testid="manager-home-approvals-unreadable">
                Antrean persetujuan tidak bisa dibaca — data gagal dimuat. Ini BUKAN
                berarti meja Anda bersih.
              </p>
            ...
            {!boardsUnreadable && (approvals.oldest || []).length > 0 && (
        ...
        <section className="section-card">
          <div className="section-head">
            ...
            <span className="text-[11px] text-[#9A9BA3]" data-testid="manager-home-late-total">
              {boardsUnreadable ? "tidak bisa dibaca" : `${late.total_items ?? 0} hal perlu dikejar`}
            </span>
          </div>
          <div className="section-body">
            {loading && !data ? (
              ...
              <p className="py-8 text-center text-[11.5px] text-[#C0392B]"
                data-testid="manager-home-late-unreadable">
                Daftar keterlambatan tidak bisa dibaca — data gagal dimuat.
              </p>
            ) : (late.rows || []).length === 0 ? (
              <p className="py-8 text-center text-[11.5px] text-[#1B7F4B]"
      ...
      {/* ── Papan antrean mahal (2026-06) ────────────────────────────────────
          PO custom · kontrabon bersengketa · retur antar-PT. Sebelum ini papan
          ini HANYA ada di Control Tower pemilik: pemilik melihat pekerjaan yang
          tanda tangannya justru ditunggu dari manajer. Komponen & angkanya sama
          persis dengan beranda pemilik (`WaitingQueueBoard` ·
          `approval_backlog_service`), jadi dua layar tak mungkin berselisih. */}
      <div className="grid gap-3" data-testid="manager-home-boards">
        {boards.map((b) => {
          ...
          return (
            <WaitingQueueBoard key={b.key} board={b} loading={loading && !data}
              ...
              testIdBase={`manager-home-board-${b.key}`}
              rowTestIdBase={`manager-home-board-${b.key}-row`} />
          );
        })}
      </div>
