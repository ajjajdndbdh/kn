...
async def manager_home(period: Optional[str] = None, entity_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    waiting_boards = await _waiting_boards(entity_id)
    day, days_in_month = _month_progress()
    return {
        "period": period,
        "special_orders_waiting": waiting_boards[0],
        "waiting_boards": waiting_boards,
        "leaderboard": board,
        "team": team,
        "totals": totals,
...
#: Antrean yang mendapat PAPAN sendiri di beranda (lengkap dengan umur tunggu).
#: Kriterianya bukan "penting" melainkan MAHAL BILA MENUNGGU: kain custom tak bisa
#: dialihkan ke pelanggan lain, kontrabon bersengketa menahan uang pemasok, dan retur
