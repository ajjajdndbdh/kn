...
Cabang "tidak bisa dibaca" ditulis **di dalam** cabang "daftar kosong". Karena `load()`
yang gagal TIDAK mengosongkan `data`, daftarnya tidak kosong → layar menyajikan antrean
**basi** dengan percaya diri dan cabang unreadable tak pernah dievaluasi. Sekarang
`boardsUnreadable` dinilai **lebih dulu** di kedua kartu (Menunggu Persetujuan Saya ·
Terlambat Hari Ini), angka KPI basi diganti `—`, dan blok antrean admin
(`admin-home-approval-backlog`) disembunyikan saat data tak terbaca.
**Pagar baru B5.2** (`backend/test_core_papan_po_custom_poc.py`): regex menuduh URUTAN
cabangnya di `AdminHome.jsx` **dan** `ManagerHome.jsx` — kelasnya, bukan satu kasusnya.

### 2. Lanjutan INV-GL-DRIFT (3 hal)
1. **Job harian `inventory_drift_watch`** (`services/inventory_drift_watch.py`) — drift
   subledger vs GL 1-1300 tidak lagi menunggu ada orang menjalankan
   `scripts/verify_data_integrity.py`. Alamatnya berbasis IZIN (`accounting.manage`,
   `create_addressed`), ambangnya milik pemilik (`inventory.drift_alert_rupiah`, bawaan
   Rp 1.000), `dedupe_scope="day"`, dan job ini **tidak memposting jurnal apa pun**.
2. **Alasan true-up sampai ke JURNALNYA** (deskripsi + field `reason`) — dulu berhenti di
   `audit_logs`, padahal yang dibaca saat tutup buku adalah jurnal.
3. **Riwayat true-up di layar** (`InventoryReconTab.jsx`): nomor · tanggal · nilai ·
   pelaku · dasar, dan "tanpa dasar tercatat" ditandai merah.

### 3. Verifikasi
`gate.sh --full` HIJAU 358 s · `test_core_sesi_2026_06_poc.py` **43 PASS · 0 FAIL**
(7 cek baru G1b) · `test_core_papan_po_custom_poc.py` **36 PASS** ·
`verify_data_integrity` PASS 242 · agen uji `iteration_249` **nol temuan**.

### 4. Jebakan lingkungan (kontainer baru)
`pip install -r backend/requirements.txt` GAGAL: baris `litellm @ <url>` bentrok dengan
`emergentintegrations==0.2.0`. Keduanya tidak dipakai kode ini — pasang requirements
tanpa dua baris itu. Bundle frontend tetap TIDAK hot-reload
...
### 0. Titik henti yang diwarisi
Sesi sebelumnya berhenti dengan **satu** gate merah:
`INV-GATE-01 — koleksi 'audit_logs': 103 -> 105 (+2)`, dan POC FASE G-8 melapor
`121 PASS · 1 FAIL`. Keduanya ternyata **bukan** dua pekerjaan, melainkan **satu POC yang
tidak membersihkan dirinya** — dan cara menemukannya bukan dengan membaca kode.

### 1. Yang dikerjakan

**(a) Residu `audit_logs +2` — diukur, bukan ditebak.** `scripts/_bisect_residu_audit.sh`
menjalankan **53 gate runtime satu per satu** dan mengukur selisih `audit_logs` sesudah
masing-masing. Tepat satu bergerak: `backend/test_core_notifikasi_alamat_poc.py` (POC FASE
N) — `login` + `special_order_created`. Diperbaiki dengan pola POC FASE S: himpunan ID
`audit_logs` + `sessions` direkam **sebelum permintaan pertama** (supaya jejak login berada
di dalam jendela pembersihan), lalu dibuang lewat **selisih himpunan** di `finally`.

**(b) FAIL POC G-8 — korban, bukan pelaku.** `verify_data_integrity` merah:
*"kanonik `permission_settings` KOSONG"*. Penyebabnya POC FASE N (N6) yang mencabut izin
lalu "memulihkan" dengan `delete_one({"id": "default"})` — benar untuk PERILAKU, **salah
untuk DATA**: baris hasil seed ikut lenyap, dan POC PERTAMA yang berjalan sesudahnya (G-8)
yang memerah. Sekarang dokumen aslinya direkam lengkap & dipulihkan apa adanya, dan
`permission_settings` **masuk `gate_residue.WATCH`** — dulu koleksi ini tidak dipantau sama
sekali, itulah sebabnya residunya bertahan.

**(c) Utang FASE N dilunasi (2 BELUM terakhir → 0).**
- `notifikasi PO custom (special_orders)`: endpoint pembuatannya sudah memberi tahu, tetapi
  hanya untuk dokumen yang lahir SESUDAH FASE N — 3 dokumen demo yang sudah menunggu tetap
  sunyi. Job notifikasi sekarang menilai **KEADAAN** (`status=pending_approval`), bukan
  kejadian: `_notify_pending_special_orders()`, alamat = `required_approval_role` +
  pemegang `order.approve`, fan-out per orang, `dedupe_scope="day"`. Dijaga POC N butir
  **N3b** (termasuk "job dua kali → tetap satu pesan per orang").
- `create_notification(recipient_permission=…)`: fakta readiness ini **salah sasaran** —
  argumen itu memang sengaja tidak dibuat (penyaring pembaca ber-**OR** akan menyiarkannya
  begitu `recipient_role` terisi). API yang nyata & dipakai 7 produsen notifikasi adalah
  `create_addressed(permission=…)`. Faktanya diubah untuk mengukur API yang ADA, dengan
  alasannya ditulis di tempat.

**(d) Cangkang setelan kosong (`KN-CFG-SHELL`).** "Kembalikan ke global" menyisakan baris
`system_settings` ber-`scope=<entitas>` yang isinya `{"ar": {}}`: `$unset` membuang NILAI,
bukan WADAH. Nol perubahan perilaku, tetapi bertambah permanen tiap override dicabut.
`clear_layer` sekarang membuang wadah kosong dan menghapus barisnya bila tak ada setelan
bermakna yang tersisa — override LAIN pada badan usaha yang sama tetap utuh (diuji 2 kasus).

### 2. JEBAKAN PENGUKURAN yang memakan waktu paling banyak (baca ini!)
Satu gate penuh MERAH dua tempat sekaligus: `system_settings 3 -> 7 (+4)` dan POC G-6b
(yang menuntut `WARN 0`) tersandung `INV-GL-DRIFT` — sementara **tiga** gate penuh lain
HIJAU dan setiap POC bersih saat dijalankan sendiri. Akarnya bukan produk:
**berkas di `backend/` disunting SELAGI gate berjalan** → uvicorn hot-reload →
`bootstrap.run_bootstrap()` jalan di tengah gate → lahir `system_settings` `makloon`,
`receiving`, `hr`, `alerts/lock` (**+4, tepat**) di antara dua pengukuran sidik jari.
`gate_residue.py` sekarang **menyebut jebakan ini di pesan gagalnya**.
**Aturan kerja: jangan menyentuh `backend/` selagi gate berjalan.**

### 3. Bukti (bukan klaim)
| Alat | Hasil |
|---|---|
| `bash scripts/gate.sh --full` | **HIJAU** 419 s · `memory/GATE_RECEIPT.md` |
| `backend/test_g8_bank_poc.py` | **122 PASS · 0 FAIL** (dulu 121 · 1) |
| `backend/test_g9_case_poc.py` | **119 PASS · 0 FAIL** |
| `backend/test_core_notifikasi_alamat_poc.py` | **35 PASS · 0 FAIL** (dulu 30) |
| `scripts/verify_data_integrity.py` | **PASS 241 · FAIL 0 · WARN 0** |
| `scripts/gate_residue.py --check` | nol residu (39 koleksi · 24 balance · 11 SO) |
| `scripts/audit_md_erp_readiness.py` | **SELESAI 96 · BELUM 0 · DRIFT 0** |

### 4. Untuk sesi berikutnya
1. **Jangan** menyunting `backend/` selagi gate berjalan (§2). Ukur dulu, sunting kemudian.
2. `INV-GL-DRIFT` (`ent_kanda` Δ900.000 persediaan subledger vs GL 1-1300) pernah muncul
   SETELAH gate, tidak pernah reproduksi dari POC uang mana pun (G-2/G-3/G-7/G-8/G-9/G-6/
   G-6b diuji satu-satu, semua `WARN 0`). Dugaan terkuat: bootstrap/scheduler yang jalan
   karena hot-reload di tengah gate. Kalau muncul lagi **tanpa** penyuntingan, mulailah dari
   `bootstrap.post_inventory_opening_balance` dan job penjadwal — bukan dari POC-nya.
3. Alat bisect sesi ini disimpan & bisa dipakai ulang untuk kelas residu apa pun:
   `scripts/_bisect_residu_audit.sh` · `scripts/_bisect_residu_settings.sh` ·
   `scripts/_bisect_gl_drift.sh` · `scripts/_intip_settings.py` (pengintai penulis baru).

---
## SESI 2026-08-24 (lanjutan · repo `kwakalsaksaia/KN`) — **FASE I DITUTUP: BUKTI DIAMBIL, BUKAN DIKLAIM**

### 0. Titik henti yang diwarisi — dan kenapa ia tampak lebih buruk dari kenyataannya
Sesi sebelumnya berhenti **di tengah satu perintah**, bukan di tengah kode. Baris
terakhirnya: `setsid nohup bash scripts/gate.sh --full …` → alat pemanggilnya kena
**`context deadline exceeded`**. Yang diwarisi karena itu bukan bug, melainkan
**ketidaktahuan**: FASE I sudah dibangun penuh tetapi tak ada receipt yang membuktikan
gate masih hijau **sesudah** tiga komponen BERSAMA disentuh (`KNSelect`, `ConfirmModal`,
`FormModal`) — dan di repo ini "selesai" tanpa receipt itu **void** (aturan §0.4).

Yang saya ukur lebih dulu, sebelum menyentuh satu baris pun:

| Diukur | Angka |
|---|---|
| `/app` saat kontainer datang | **template KOSONG** (`server.py` 88 baris "Hello World", `App.js` 56 baris) — repo GitHub belum ada di dalamnya |
| `git clone` + rsync (kecuali `.env`, `node_modules`, `.git`) | repo utuh masuk: backend 923-baris `inspection_service.py`, layar `features/inspections/` 4 berkas |
| `.restore_env.sh` | HIJAU ujung-ke-ujung: `SEED_EXIT=0` · fondasi LENGKAP (`expense_categories` 8 · `gl_accounts` 75 · `uoms` 8) · `BUILD_EXIT=0` |
| `.emergent/emergent_todos.json` | **17 `completed`**, 1 `in_progress` (gate ULANG), 1 `pending` (dokumentasi) → pekerjaan kode FASE I memang sudah tuntas |
| `audit_md_erp_readiness` | `SELESAI=92 · BELUM=3 · DRIFT=1` — dan **ketiga BELUM + satu DRIFT milik FASE N**, nol milik I |

### 1. Yang dikerjakan sesi ini
**(a) Pelari gate yang tidak bisa dipanggil dua kali.** Akar `context deadline exceeded`
bukan gate-nya (340 s memang wajar), melainkan **alat pemanggil yang mengulang perintah
saat timeout**. `gate.sh` sudah punya kunci `/tmp/kn_gate.lock` yang menolak gate kedua —
tetapi penolakan itu terjadi **sesudah** proses kedua lahir, dan log/receiptnya tetap
tercampur. Jadi lapisan kedua ditambahkan: `.logs/run_gate_bg.sh` — memisahkan diri dari
sesi pemanggil (`setsid`), menaruh PID pelari di `/tmp/kn_gate_runner.pid`, **langsung
menolak** bila pelari lain masih hidup, menulis log per label dan **penanda `.done`
berisi kode keluar** supaya kemajuan gate bisa ditanya tanpa menahan alat pemanggil.
Ini yang mengubah "gate tak bisa dijalankan sampai selesai" menjadi dua jalan penuh.

**(b) Dua `gate.sh --full` HIJAU BERTURUT** — **340 s** dan **341 s**, receipt **111
baris, 0 FAIL**, dengan data demo dipulihkan lebih dulu (`seed_realistic.py` +
`seed_e9_chain_demo.py`). Ini menutup satu-satunya todo `in_progress` yang diwarisi.

**(c) Dokumentasi yang tertinggal DUA sesi** akhirnya ditulis: `plan.md` **§STATUS I**
(sebelumnya plan.md berhenti di §STATUS S sehingga seluruh FASE I tak terbaca di roadmap),
`SESSION_HANDOFF.md` (header masih 2026-08-21 — dua sesi tak tercatat), `test_result.md`
(angka disegarkan ke hasil terukur hari ini), `memory/test_credentials.md` (**bagian FASE I**:
jalan navigasi ke SPK Inspeksi + keadaan awal 3 dokumen demo + jebakan testid).

### 2. Bukti penutup (angka, bukan klaim)
* `gate.sh --full` **HIJAU 2× BERTURUT** — 340 s & 341 s · receipt **111 gate PASS / 0 FAIL**
* POC FASE I `backend/test_core_inspeksi_poc.py` **93 PASS / 0 FAIL** (I1–I12), nol residu
* `verify_data_integrity` **241 PASS / 0 FAIL / 0 WARN**
* `audit_doc_refs --strict` HIJAU · `INV-GATE-01` anti-residu HIJAU (stok dipulihkan
  EKSAK: 334 dokumen di 8 koleksi) · `audit_endpoint_sweep` **472 GET · 5xx = 0**
* `audit_md_erp_readiness --fase I`: **seluruh baris I SELESAI** (termasuk `qc_inspections`
  DICABUT, `INV-QC-02` dua arah, milestone retur **3/3** dokumen, `REL_LABEL ⊆ REL_INVERSE`
  **18 relasi berpasangan** → utang **D2** dan **D6** LUNAS)

### 3. Jebakan untuk sesi berikutnya (BACA)
* **Kontainer datang KOSONG.** Urutannya wajib: `git clone` → rsync **tanpa** `.env`
  (`MONGO_URL`/`REACT_APP_BACKEND_URL` tak boleh tertimpa) → `bash .restore_env.sh`.
  Jangan menyimpulkan apa pun tentang kode sebelum langkah ini hijau.
* **Jangan pernah memanggil `gate.sh` langsung dari alat ber-timeout.** Pakai
  `bash .logs/run_gate_bg.sh <label> --full`, lalu tanya `.logs/gate_<label>.done`.
  Memanggil langsung = risiko dua gate = **30+ merah palsu** (pelajaran 2026-08-23).
* **Banyak POC merah sekaligus? Baca yang PERTAMA merah.** POC yang crash tidak
  membersihkan dirinya, dan residunya memerahkan POC-POC sesudahnya di gate yang sama
  (terukur 9 gema di FASE S).
* **Utang yang tersisa semuanya milik FASE N**: `services/notification_audience.py` (BELUM) ·
  `create_notification(recipient_permission=…)` (BELUM) · notifikasi PO custom
  `special_orders` (BELUM) · **D5** 11 notifikasi ber-`recipient_role="all"` (DRIFT).

### 4. Berikutnya
**FASE N — NOTIFIKASI YANG BERALAMAT** (§N `RENCANA_EKSEKUSI_MD_ERP.md`): pemberitahuan
tak lagi dikirim ke `"all"` tetapi ke **peran + izin + badan usaha** yang benar, plus
D5 (11 notifikasi warisan) dibereskan. Sesudahnya **M (Makloon)**.

---
## SESI 2026-08-21 (lanjutan · repo `sjdjchdhhd/kn`) — **INV-REF-04 LAHIR · RESIDU POC DITUTUP SAMPAI AKAR**

### 0. Pemulihan lingkungan (wajib tiap clone — kontainer datang KOSONG)
Kontainer datang sebagai **template kosong** (`server.py` Hello World, Mongo kosong) dan repo
`sjdjchdhhd/kn` **private** → `git clone` gagal minta login. Pemilik menjadikannya public,
lalu: `git clone` → `git fetch` ke `/app` → `git reset --hard` (**`.env` selamat karena
di-.gitignore**) → `bash /app/.restore_env.sh` (hijau: deps · mongo hidup · bootstrap ·
seed · verifikasi fondasi · build FE). `memory/test_credentials.md` **ikut ada di commit
WIP** kali ini, jadi tidak perlu ditulis ulang.

### 1. Titik henti yang diwarisi — satu baris merah, semua alat lain HIJAU
```
python scripts/audit_doc_refs.py --strict   →  ✗ [FAIL] Tautan menggantung: 2 temuan
```
POC E-7 **58/58 hijau** · `verify_data_integrity` **237/0/0 hijau**. Itu sebabnya sesi
sebelumnya berhenti: alatnya saling bertentangan dan tak ada yang menjelaskan.

Reproduksi (bukan dugaan): dari seed bersih `audit_doc_refs` **HIJAU**; sesudah satu kali
POC E-7 → **2 hantu**, keduanya milik `internal_requests KSC/PIN-00003 --fulfilled_by-->
interco_transaction` yang sudah dihapus pembersih POC.

### 2. Akar: kewajiban yang lahir dari `link()` dua arah tetapi tak pernah ditulis
`doc_refs_service.link()` menulis relasi **DUA ARAH**. Konsekuensinya: **siapa pun yang
menghapus dokumen wajib menyapu ref yang menunjuk dokumen itu.** Tidak ada satu baris pun
di repo yang menyatakan itu — jadi lubangnya ada di mana-mana, termasuk **PRODUKSI**:

* `po_amendment_service` menghapus inbound `wms_tasks` (doc_type `grn`) yang **sudah bertaut
  ke PO-nya** → tiap amandemen PO meninggalkan PO hidup dengan tautan **404**;
* `purchase_return_service.delete_purchase_return` (PO memegang `reversed_by` ke retur mati);
* `routers/supplier_contracts.delete_contract` (order makloon menaut lewat `contract_id`
  tingkat atas, pagar hapus hanya memeriksa `steps.contract_id`);
* `routers/sales_orders` rollback transfer — **2 jalur `except` kembar**, disatukan jadi
  `_rollback_created_transfers()` supaya sapuannya tak mungkin lupa disalin ke salah satunya.

### 3. Yang dibangun
1. **`doc_refs_service`**: `unlink_all()` · `safe_unlink_all()` · `dangling_refs()` — satu
   definisi yang dipakai bersama `audit_doc_refs`, `gate_residue`, dan penjaga baru
   (sebelumnya `audit_doc_refs` punya salinan logikanya sendiri, dan celah "hampir sama"
   itulah yang meloloskan residu berbulan-bulan).
2. **`INV-REF-04`** `scripts/guardrails/verify_ref_unlink.py` — statik lewat `ast`, resolusi
   konstanta **lintas-modul** (`db[cs.COLL]`), koleksi dinamis wajib berikut **alasan
   tertulis**, `--self-test` **8 kasus dua arah**. Bukti-merah pada kode sebelum perbaikan:
   **5 pelanggaran produksi**.
3. **`poc_stock_guard.sweep_ghost_refs()`** — satu baris di cleanup POC. Sengaja TIDAK
   dipakai di produksi: di sana ref yatim adalah GEJALA yang harus diperbaiki di titik
   hapusnya, bukan disapu diam-diam.
4. **`scripts/ukur_residu_poc.py`** (alat baru) — jalankan tiap POC gate satu per satu,
   hitung selisih **SELURUH koleksi**. Dibuat karena `gate_residue --check` hanya bisa
   berkata "blok POC meninggalkan residu" tanpa menunjuk POC MANA; menebak dari nama
   koleksi sudah **dua kali** salah.
5. **`gate_residue.py`** — daftar pantauan **+11 koleksi** + pemeriksaan **tautan
   menggantung** yang **tidak pernah** diabaikan `--ignore-trails`.

### 4. Temuan terbesar sesi ini (yang tak dicari, tetapi paling mahal)
Pengukuran menyeluruh membuka bahwa **14 dari 33 POC** meninggalkan residu di koleksi yang
tak dipantau siapa pun. Satu kali `gate.sh --full` menyuntik permanen ke buku demo:
**+9 jurnal · +7 transaksi kas · +2 pinjaman antar-PT · +2 aset tetap · +1 eliminasi
konsolidasi · +2 PR** (dari POC E-7 saja), plus `store_credit_ledger` (**saldo uang
pelanggan**), `system_settings` (**SSOT perilaku aplikasi**), `credit_overrides`
(**pembebasan pagar kredit**), dan `approval_matrix_log`. POC E-7 bahkan menyebut ini
"SENGAJA ditinggalkan karena uangnya sudah berpindah — append-only": alasan itu benar untuk
menolak "hapus dokumennya saja" (jurnal jadi yatim), tetapi **salah** sebagai kesimpulan —
obat yang benar adalah memulihkan **kedua sisi pada satu saat yang sama**.

### 5. Bukti
`gate.sh --full` **106 gate HIJAU / 0 FAIL, 2× BERTURUT** · POC E-7 **62/62** (2× berturut,
angka koleksi kembali PERSIS) · `audit_doc_refs --strict` **32/0** · `verify_data_integrity`
**237/0/0** · `INV-REF-04` self-test **8/8** · `ukur_residu_poc` **nol residu non-jejak di
33 POC**. Diwarisi & diverifikasi (bukan dikerjakan sesi ini): **P-0** (POC 36/36,
`INV-ORIG-01`) dan **FASE P** Papan PO per Lini (`INV-STAGE-01`).

### 6. Jebakan untuk sesi berikutnya (BACA)
* POC yang **MENGHAPUS dokumen bernomor** wajib `sweep_ghost_refs()`; POC yang **BERJURNAL**
  wajib `snapshot_stock(FULL_COLLECTIONS)` (stok **dan** jurnal **dan** kas).
* **`number_sequences` JANGAN pernah dipulihkan** — penghitung nomor dokumen harus monoton;
  memulihkannya = memakai ulang nomor surat (kelas INV-NUM-01).
* **`asyncio.run()` sekali saja per proses POC.** Klien motor di `db.py` mengikat diri ke
  event loop pertama; pemanggilan kedua mati `RuntimeError: Event loop is closed`. Kalau itu
  terjadi di dalam blok pembersihan, POC **menambah** residu (terukur +17 `audit_logs`)
  sambil tetap melaporkan "0 FAIL".
* **Residu satu-kali gampang menipu alat ukur**: dokumen scope `system_settings`
  (`receiving`, `ent_ksc`) hanya lahir pada jalan PERTAMA, jadi pengukuran di DB yang sudah
  pernah dipakai akan berkata "bersih". Reset dulu (`system_settings` ke `global/uom/lot`)
  sebelum menyimpulkan.
* **`gate_residue --check` menunjuk GEJALA, bukan pelakunya** — untuk pelakunya pakai
  `scripts/ukur_residu_poc.py`.

### 7. Berikutnya
**FASE S (Sampling)** → **I (Inspeksi/QC)** → **N (Notifikasi)** → **M (Makloon)**.
Utang DRIFT: D2 `qc_inspections` hantu (I) · D5 11 notifikasi `recipient_role="all"` (N) ·
D6 `REL_LABEL` memuat `applied_to` yang tak ada di `REL_INVERSE` (I) · D7 nomor dokumen
kembar di `md_samples` (S).

---
# SESSION HANDOFF — Kain Nusantara (WMS/ERP) — arsip sesi sebelumnya
...
---
## SESI 2026-08-20 (repo `yavabasakangja/kn`) — **FASE D DITUTUP** (Permintaan Desain + peran ke-7 `designer`)

Titik henti yang diwariskan: di tengah `grep "### internal_requests" ENTITY_REGISTRY.md`
— yaitu sedang mencari **template entri registry** untuk mendaftarkan `design_requests`.
Permintaan pemilik: *"lanjutkan development dari repo ini, sebelumnya development terhenti disini."*

### 0. Pemulihan lingkungan (wajib tiap clone — kontainer datang KOSONG/template)
`git clone github.com/yavabasakangja/kn → /tmp/kn_probe` → `rsync` ke `/app`
(**JANGAN** timpa `backend/.env` & `frontend/.env`; keduanya di-.gitignore) →
`bash /app/.restore_env.sh`. `memory/test_credentials.md` di-.gitignore → **ditulis ulang**.

⚠️ **KONTAINER DATANG DENGAN `mongodb` MATI** (supervisor: STOPPED) — dan itu memakan
waktu saya. Versi lama `.restore_env.sh` tidak memeriksanya, jadi langkah "restart backend"
menjalankan `bootstrap.run_bootstrap()` **ke ketiadaan**, lalu seed mati dengan
`ServerSelectionTimeoutError`. Sesudah mongo dinyalakan tangan + seed diulang, semuanya
**TAMPAK** sehat — padahal koleksi FONDASI (`expense_categories`, `gl_accounts`,
`hr_employees`) tetap KOSONG karena bootstrap **hanya jalan saat backend START**.
Akibatnya **2 dari 9 gate merah yang saya kejar BUKAN BUG KODE** (lihat §3).
**`.restore_env.sh` sudah dikeraskan**: mongo dipastikan hidup → backend restart →
seed → **VERIFIKASI FONDASI yang gagal-berisik** (7 langkah, bukan 5).

### 1. Yang terukur di titik henti (bukan ditebak)
`gate.sh --full` → **MERAH 9 dari 120**. FASE D terbangun sebagian: koleksi
`design_requests` (4 dok) · service · router · 4 layar FE · akun `designer@` · izin · nav
hub. **Belum ada** POC (§D.D mewajibkannya) dan **belum masuk** `ENTITY_REGISTRY.md`.

### 2. Sembilan gate merah = TIGA akar
* **A · peran ke-7 lahir tanpa didaftarkan di tempat yang MENGHITUNG peran.**
  `check_nav_map.py` menyimpan `ROLES` **hardcode 6 nama** sementara beranda peran dibaca
  dari SSOT → `KeyError: 'designer'` dan gate **MELEDAK** (bukan merah dengan penjelasan;
  CHECK 4 & 5 tak pernah jalan). POC E-8 G1 + `role_registry --self-test` mengunci
  **angka 6**. `audit_sales_roles_ux` menemukan **3 panel mati** milik designer.
* **B · pintu keputusan `design_request` tak terdaftar** di `DOOR_QUEUE` (INV-APPR-01) &
  `verify_home_kpi.EXPECT` (INV-HOME-01) → merembes ke POC F-6 (4) & F-6.7 (2).
* **C · dua gate merah yang TERBUKTI artefak lingkungan** (§0), hijau tanpa satu baris
  kode aplikasi diubah.

### 3. Perbaikan yang penting diketahui
* **Daftar peran tidak boleh di-hardcode lagi.** `check_nav_map.py` MEMBACA
  `frontend/src/config/roles.js ROLE_REGISTRY`, dan **MELEDAK BERISIK** (`exit 2`) bila
  blok itu tak terbaca — menolak daftar cadangan, karena cadangan = sumber kedua.
  POC E-8 kini menguji **paritas server↔layar**, bukan populasi peran (daftar-putih nama
  peran di parser FE — sumber KEEMPAT — dihapus, sehingga peran TERBARU justru ikut diuji).
* **Dua panel mati diperbaiki DI AKARNYA, bukan dengan pembebasan.** `/suppliers` &
  `/warehouses` **menumpang** di modul bersama `features/rnd/rndApi.js` yang diimpor layar
  milik designer padahal tak pernah dipanggil (`listWarehouses` **NOL PEMAKAI**;
  `listSuppliers` hanya dipakai `SampleSendModal.jsx`). Pembebasan akan menghijaukan gate
  **atas alasan yang salah**. → endpoint dipindah ke dekat pemakainya.
* **Panel mati ke-3 disembuhkan dengan MEMBERI:** `GET /api/design/reports/mine` dipagari
  `design_request.**view**` (bukan `.report`) → tab **"Rapor Saya"**. Melihat angka DIRI
  SENDIRI bukan wewenang penilaian. `items` absen, nol nama rekan (PS-18).
* **POC baru `backend/test_core_design_request_poc.py` — 55/55, 4×, nol residu**, terdaftar
  di `gate.sh` sebagai **`POC FASE D · PERMINTAAN DESAIN (DSR)`**.

### 4. Bukti
`gate.sh --full` **HIJAU 2× berturut** (309s & 311s · **145 PASS / 0 FAIL**) ·
`verify_data_integrity` **237/0/0** · `validate_compliance` **22 PASS / 0 FAIL** ·
`audit_sales_roles_ux` **HIJAU (7 peran, nol layar/panel mati)** ·
`audit_md_erp_readiness --fase D` **SEMUA SELESAI** · build FE bersih ·
**D-US1..D-US4 dijalankan SENDIRI di peramban** (DSR-00014 dibuat & ditugaskan;
DSR-00003 diminta revisi ber-alasan lalu diserahkan ulang; Rapor Desainer & Rapor Saya
terisi) · agen uji iterasi **236 backend 18/20 nol bug kritis** & **237 frontend 7/9**
dengan konfirmasi **"zero dead panels — nol 403"** dan **regresi bersih**.
Data demo dipulihkan ke **4** dokumen `design_requests`.

### 5. Jebakan untuk sesi berikutnya (BACA)
* **`KNSelect` merender placeholder SEBAGAI OPSI** (`{testId}-option-empty`). Memilihnya
  MENGOSONGKAN pilihan → tombol simpan mati. Pakai testid kanonik
  **`{testId}-option-{value}`**, jangan `[role="option"]` generik. Ini sudah memproduksi
  satu laporan bug palsu dari agen uji (saya reproduksi: alurnya **normal**).
* **Ada DUA "POC FASE D"** di `gate.sh` (Permintaan Desain vs makloon rantai proses,
  penomoran lama). Label sudah dibedakan.
* **Gate/POC merah dengan gejala "0 baris global" atau "404 profil karyawan" =
  LINGKUNGAN, bukan kode.** Jalankan `supervisorctl restart backend` lalu ukur ulang.
* **Agen uji meninggalkan dokumen `KSC/DSR-00005` (dua kali).** Selalu periksa residu
  sesudah memanggilnya; pemulihan `seed_realistic.py` → `seed_e9_chain_demo.py`.
* **Endpoint baru jangan ditaruh di modul API bersama** kalau hanya satu layar
  memakainya — `audit_sales_roles_ux` menilai layar dari penutupan impornya.
* `gate_residue --check` hanya sah dibandingkan **pada jendela yang sama**; sesudah
  re-seed, baseline-nya perlu `--save` (pelajaran yang sama dengan sesi FASE U).

### 6. Berikutnya
**P-0 — tautan PO→PR** (keputusan pemilik: **hanya dokumen BARU**, tanpa backfill) —
prasyarat FASE P, karena terukur **0/14** PO menyimpan `pr_id` sehingga "Nama Sales" di
papan PO mustahil dirunut. Sesudahnya **S (Sampling)** → **I (Inspeksi/QC)** →
**P (Papan PO)** → **N (Notifikasi)** → **M (Makloon)**.

### 7. KREDENSIAL UJI
Semua sandi **`demo12345`**. Peran ke-7 baru: **`designer@kainnusantara.id`**
(Sari Melati). Selengkapnya di `memory/test_credentials.md`.
Login API: `POST /api/auth/login` → respons memakai key **`token`** (bukan `access_token`).

---

# SESI 2026-08-23 — **FASE S DITUTUP** (lanjutan repo `skkajdhs/KN` sesudah WIP terhenti)

> Laporan lengkap: `plan.md` **§STATUS S**. Di sini hanya yang perlu dibaca cepat oleh
> sesi berikutnya.

### 1. Titik henti yang diwarisi
Commit `48b3abd "WIP: simpan progress saya"` berhenti **di tengah frontend FASE S**
(editan terakhir: menambah `import { roundTypeOf }` di `RndReportsView.jsx`). Backend
FASE S sudah selesai — POC `test_core_sampling_poc.py` **65/65** begitu lingkungan
dipulihkan (`.restore_env.sh` → `BUILD_EXIT=0`).

### 2. Yang dikerjakan
* **Frontend pembaca plural** (6 berkas): `RndReportsView` · `RndSpecsView` ·
  `SpecFormModal` (pilihan jenis + "wajib desain" dari MASTER, bukan `TYPE_OPTS`
  hardcode) · `SpecDetailPanel` · `RndSamplesView` (deep-link tak lagi memaksa
  `["proofing"]`) · `designer/DesignerSlaPanel`.
* **`sampleTypeMeta.typeOptions()` + `FALLBACK_TYPE_CODES`** — cadangan hanya jenis baku
  yang masih AKTIF (`bulk_sample` tidak ditawarkan, tetapi labelnya tetap ada untuk
  dokumen lama).
* **`rnd_sla_service` mengirim `sample_type_label`** dari master (termasuk jenis
  nonaktif) — karena papan eskalasi hidup di menu **Desainer**, dan peran `designer`
  **tidak punya `rnd.view`**: kalau layarnya memanggil `/api/rnd/sample-types` sendiri,
  panelnya mati 403.
* **Dua pembaca LAMA yang membuat `gate --full` MERAH** (bukan bug pekerjaan frontend):
  `INV-RND-02` masih menuntut nomor round berurut **per supplier** (sejak FASE S
  per **supplier × jenis** — data demo sehat dituduh "[1, 1]"), dan **POC FASE F**
  masih menuntut `bulk_sample` ada di enum + mengirim hasil ukur pilihannya sendiri
  (`gsm_actual` bukan hasil ukur labdip) sehingga server menjawab 400.

### 3. Bukti
`gate.sh --full` **HIJAU 2× berturut (339s & 340s)** · POC S **65/65** ·
POC F **100/0** (dari crash) · `verify_data_integrity` **237/0/0** ·
`audit_md_erp_readiness --fase S` **nol BELUM/DRIFT** · `validate_compliance` **22/0** ·
agen uji iterasi **238** (S.F-1 lulus, nol bug) + uji layar sendiri untuk S.F-2/3/4 dan
3 layar lama tanpa regresi.

### 4. JEBAKAN BARU (yang paling mahal sesi ini)
* **POC yang CRASH tidak menjalankan cleanup-nya.** Satu `KeyError` di POC FASE F
  meninggalkan residu (3 `sales_orders` data demo terhapus) yang memerahkan **9 POC
  berikutnya di gate yang SAMA** (G-2·G-3·G-4·G-6b·G-7·G-8·G-9·P·F-US3) + `INV-GATE-01`
  + `verify_data_integrity`. **Kalau banyak POC merah sekaligus: baca yang PERTAMA
  merah**, perbaiki akarnya, `python seed_realistic.py`, lalu ukur ulang. 9 dari 13
  kegagalan hari ini adalah gema, bukan bug.
* **Perubahan tunggal→daftar juga menua-kan ALAT**, bukan hanya kode: invarian
  (`verify_data_integrity`) dan **POC fase lama** yang menuliskan bentuk data lama.
  Keduanya hanya terlihat di `gate.sh --full`.
* **`_validate_measurements` menolak field yang tidak diminta master.** Skrip/seed/POC
  yang menulis daftar hasil ukurnya sendiri akan memerah begitu master diubah — baca
  `measurement_fields`, jangan menyalinnya.

### 5. Berikutnya
**FASE I — Inspeksi & QC sebagai dokumen** (keputusan pemilik #5 dikonfirmasi ulang
2026-08-23: **warna beda = barang DITAHAN · handfeel beda = PERINGATAN · pelepas
tahanan = MANAJER**), lalu **N (Notifikasi)** → **M (Makloon)**.
